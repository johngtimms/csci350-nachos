...
// Resource names cannot be longer than 32 characters
// vaddr is the Nachos address register pointing to the name
char* argumentReader(unsigned int vaddr, char *identifier) {
    char *name = new char[32+1];
    if( copyin(vaddr,32,name) == -1 ) {
...
int genericFunction(char *message, char *identifier, int action) {
    ...
    if(which == SyscallException) {
        switch (type) {
            ...
            case SC_Wait:
                ...
                Wait_Syscall(machine->ReadRegister(4), machine->ReadRegister(5), machine->ReadRegister(6), machine->ReadRegister(7));
                break;
            case SC_Signal:
                DEBUG('a', "Signal syscall.\n");
                Signal_Syscall(machine->ReadRegister(4), machine->ReadRegister(5), machine->ReadRegister(6), machine->ReadRegister(7));
                break;
            case SC_Broadcast:
                DEBUG('a', "Broadcast syscall.\n");
                Broadcast_Syscall(machine->ReadRegister(4), machine->ReadRegister(5), machine->ReadRegister(6), machine->ReadRegister(7));
                break;
            case SC_Print:
