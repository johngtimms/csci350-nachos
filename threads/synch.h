...
// the semaphore directly -- even if you did read the value, the only thing 
// you would know is what the value used to be.  You don't know what the 
// value is now, because by the time you get the value into a register, a 
// context switch might have occurred, and some other thread might have called 
// P or V, so the true value might now be different.
...
class Semaphore {
	...
	public:
		...
		// These are the only operations on a semaphore. They are both atomic.
		void P();
		void V();
...
//
//   Release -- set lock to be FREE, waking up a thread waiting in Acquire 
//              if necessary
//
...
class Lock {
	...
	public:
		...
		// These are the only operations on a lock. They are both atomic.
		void Acquire();
		void Release();

		// Return true if the current thread holds this lock. Useful for
		// checking in Release() and in condition variable operations.
		bool isHeldByCurrentThread();

	private:
	
		char* name;							// for debugging
		char* holderName;					// name of thread holding the lock
		bool value;							// lock value FREE (false) or BUSY (true)
...
//
// All operations on a condition variable must be made while the current 
// thread has acquired a lock.  Indeed, all accesses to a given condition 
// variable must be protected by the same lock. In other words, mutual 
// exclusion must be enforced among threads calling the condition variable 
// operations.
//
...
class Condition {
	...
	public:
		...
		char* getName() { return (name); }	// debug aid

		// These are the three operations on condition variables. Releasing
		// the lock and going to sleep are atomic in Wait(). conditionLock
		// must be held by the currentThread for Signal() and Broadcast().
	...
	private:
		...
		char* name;							// debug aid
		List *queue;						// threads waiting for signal / broadcast
